...
struct DeferredInitialization
{
    ...
    void initialize(Args... args) { new ((void*)&data) T(args...); initd = 1; }
    void deinitialize() { ((T&)*this).~T(); initd = 0; }

    operator T&() { return *reinterpret_cast<T*>(&data); }
    operator const T&() const { return *reinterpret_cast<const T*>(&data); }
};
template<typename T>
struct DeferredSingleton
{
    ...
    template<typename... Args>
    bool initializeStaticInstance(Args... args)
    {
        if (sInstance.isInitialized()) return false;
        sInstance.initialize(args...);
        return true;
    }
    template<typename... Args>
    bool deinitializeStaticInstance(Args... args)
    {
        if (!sInstance.isInitialized()) return false;
