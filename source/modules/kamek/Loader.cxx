...
constexpr u32 resolveAddress(u32 address, u32 text) {
  return ((address & 0x80000000) || (address & 0x90000000)) ? address
                                                            : text + address;
}

static bool handleRelocElf(k cmd, addr& address, u32 resolved,
                           const u32* input) {
#ifdef KAMEK_VERBOSE
...
bool handleElfRelocation(const u8*& input, u32 text) {
  ...
  const u32 resolved = cmd == k::ExternRel24
                           ? *(const u32*)input
                           : resolveAddress(*(const u32*)input, text);
...
KamekLoadResult loadKamekBinary(KamekLoadParam param) {
  const KBHeader* header =
      reinterpret_cast<const KBHeader*>(param.binary.data());

  if (!header)
    return KamekLoadResult::MalformedRequest;
  ...
  u32 text = reinterpret_cast<u32>(
      mem::Alloc(header->codeSize + header->bssSize,
                 param.heap ? *param.heap : mem::GetDefaultHeap()));
  ...
  const u8* inputEnd =
      reinterpret_cast<const u8*>(param.binary.data() + param.binary.size());
